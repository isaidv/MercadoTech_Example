...
- **IA**: `voyage-4-lite` de Voyage AI (embeddings, 1024 dims) + `claude-haiku-4-5`
  de la API de Claude (chat) — dos proveedores porque la API de Claude no genera
  embeddings, con capa de abstracción para cambiar de proveedor sin tocar el
  resto del código
...
- Un proveedor no cubre todo el RAG: la API de Claude redacta pero **no genera
  embeddings**, así que la mitad vectorial la hace Voyage AI. Cada proveedor vive
  en su propio archivo de `lib/ai/` y los errores siempre nombran cuál falló.
- Los embeddings van por `fetch` a la API de Voyage y el chat por el SDK oficial
  `@anthropic-ai/sdk` — al revés que en ReadHub, y por razones de cada proveedor.
- `input_type` (`document` al indexar, `query` al buscar) es asimétrico y
  obligatorio: equivocarlo no da error, solo degrada la búsqueda en silencio.
- La dimensión del vector (1024) queda fijada en la columna SQL: cambiar de modelo
  de embeddings implica migración (`ALTER COLUMN ... TYPE vector(N)` + recrear
  índice y función).
- Los umbrales de similitud no se heredan entre modelos de embeddings: se
